...
object Adts {
  ...
  def getElementN(list: List[Int], n: Int): Option[Int] = if (list.length - 1 < n) {
    None
  } else {
    Some(list(n))
  }
  ...
  def testGetNth(list: List[Int], n: Int): Option[Int] = getElementN(list, n)

  // b) Напишите функцию, увеличивающую число в два раза.

  def doubleNumber(n: Option[Int]): Option[Int] = n match {
    case Some(a) => Some(a * 2)
    case None => None
  }

  // примените функцию из пункта (b) здесь, не изменяйте сигнатуру
  def testDouble(n: Option[Int]): Option[Int] = doubleNumber(n)
  ...
  def isEven(n: Int): Either[String, Int] = if (n % 2 == 0) Right(n) else Left("Нечетное число " + n.toString)
  ...
  def safeDivide(a: Int, b: Int): Either[String, Int] = try {
    Right(a / b)
  } catch {
    case _: java.lang.ArithmeticException => Left("Нельзя делить на 0")
  }
