...
object Maps {
   ...
   */
  def testGroupUsers(users: Seq[User]): Map[String, Int] = users.groupBy(user => user.name).map(group => {
    (group._1, group._2.map(u => u.age).sum / group._2.length)
  })
   ...
   *    Вы можете реализовать ваше решение в теле тестовой функции. Не изменяйте сигнатуру.
   */
  def testNumberFrodos(map: Map[String, User]): Int = map.count(m => m._2.name.indexOf("Adam") != -1)

  /* c) Удалите всех пользователей возраст которых менее 35 лет.
   *    Вы можете реализовать ваше решение в теле тестовой функции. Не изменяйте сигнатуру.
   */
  def testUnderaged(map: Map[String, User]): Map[String, User] = map.filter(m => m._2.age >= 35)
}
